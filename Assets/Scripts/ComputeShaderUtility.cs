...
{
    public uint dispatchX;
    public uint dispatchY;
    public uint dispatchZ;

    public override string ToString() 
    {
        return string.Format("({0}, {1}, {2})", dispatchX, dispatchY, dispatchZ);
    }
}
    ...
    public static DispatchDimensions CheckGetDispatchDimensions(ComputeShader shader, int handle, uint desired_threadNum_x, uint desired_threadNum_y, uint desired_threadNum_z)
       {
           ...
           dp.dispatchX = desired_threadNum_x / group_size_x;
           dp.dispatchY = desired_threadNum_y / group_size_y;
           dp.dispatchZ = desired_threadNum_z / group_size_z;
